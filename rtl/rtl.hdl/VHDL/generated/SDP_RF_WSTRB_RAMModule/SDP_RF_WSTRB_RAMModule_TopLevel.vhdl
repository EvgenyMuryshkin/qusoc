...
begin
	...
	process (Clock, Inputs_WSTRB(0), Inputs_WriteAddress, Inputs_WriteData, State_Buff0, Inputs_ReadAddress)
	begin
		if rising_edge(Clock) then
			if Inputs_WSTRB(0) = '1' then
				State_Buff0(TO_INTEGER(Inputs_WriteAddress)) <= Inputs_WriteData(0);
			end if;
			State_ReadData(0) <= State_Buff0(TO_INTEGER(Inputs_ReadAddress));
		end if;
	end process;
	-- inferred simple dual port RAM with read-first behaviour
	process (Clock, Inputs_WSTRB(1), Inputs_WriteAddress, Inputs_WriteData, State_Buff1, Inputs_ReadAddress)
	begin
		if rising_edge(Clock) then
			if Inputs_WSTRB(1) = '1' then
				State_Buff1(TO_INTEGER(Inputs_WriteAddress)) <= Inputs_WriteData(1);
			end if;
			State_ReadData(1) <= State_Buff1(TO_INTEGER(Inputs_ReadAddress));
		end if;
	end process;
	-- inferred simple dual port RAM with read-first behaviour
	process (Clock, Inputs_WSTRB(2), Inputs_WriteAddress, Inputs_WriteData, State_Buff2, Inputs_ReadAddress)
	begin
		if rising_edge(Clock) then
			if Inputs_WSTRB(2) = '1' then
				State_Buff2(TO_INTEGER(Inputs_WriteAddress)) <= Inputs_WriteData(2);
			end if;
			State_ReadData(2) <= State_Buff2(TO_INTEGER(Inputs_ReadAddress));
		end if;
	end process;
	-- inferred simple dual port RAM with read-first behaviour
	process (Clock, Inputs_WSTRB(3), Inputs_WriteAddress, Inputs_WriteData, State_Buff3, Inputs_ReadAddress)
	begin
		if rising_edge(Clock) then
			if Inputs_WSTRB(3) = '1' then
				State_Buff3(TO_INTEGER(Inputs_WriteAddress)) <= Inputs_WriteData(3);
			end if;
			State_ReadData(3) <= State_Buff3(TO_INTEGER(Inputs_ReadAddress));
		end if;
	end process;
